...
def group_by_centroid(restaurants, centroids):
    ...
    """
    """pairs = pairs of centroid-restaurant groupings"""
...
def find_centroid(cluster):
    ...
    """List of all restaurant locations"""
    locations = [restaurant_location(restaurant) for restaurant in cluster]

    """List of all restaurant 'x' coordinates"""
    locations_lattitude = [locations[x][0] for x in range(len(locations))]

    """List of all restaurant 'y' coordinates"""
    locations_longitude = [locations[x][1] for x in range(len(locations))]

    return [mean(locations_lattitude), mean(locations_longitude)]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xy = sum([(x[0] - mean(xs)) * (x[1] - mean(ys)) for x in xy])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    predictor_only_list = predictor_list[0]
    r_squared_only_list = predictor_list[1]

    max_feature = max(feature_fns, key = lambda feature : find_predictor(user, reviewed, feature)[1])
